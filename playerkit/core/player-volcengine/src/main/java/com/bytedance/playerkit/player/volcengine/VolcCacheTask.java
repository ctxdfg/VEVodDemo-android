...
class VolcCacheTask implements CacheLoader.Task {
    ...
    @Override
    public void selectTrack(@Track.TrackType int type, @Nullable Track track) {
        synchronized (this) {
    ...
    private void preloadVid(MediaSource source) {
        final String id = source.getMediaId();
        final String playAuthToken = source.getPlayAuthToken();
        final PreloaderVidItem preloadItem = new PreloaderVidItem(id, playAuthToken, false);
        preloadItem.mPreloadSize = DEFAULT_PRELOAD_SIZE; // must
    ...
    private void preloadUrl(MediaSource source) {
        ...
        final String cacheKey = mCacheKeyFactory.generateCacheKey(source, target);
        final long preloadSize = resolvePreloadSize(target);

        final PreloaderURLItem preloadItem = new PreloaderURLItem(cacheKey, id, preloadSize, new String[]{target.getUrl()});
    ...
    private Track resolvePreloadTrack(MediaSource source) {
        ...
        if (target == null) {
            if (mTrackSelector != null) {
                target = mTrackSelector.selectTrack(TrackSelector.TYPE_PRELOAD, Track.TRACK_TYPE_VIDEO, tracks);
            }
            if (target == null) {
                target = tracks.get(0);
            }
