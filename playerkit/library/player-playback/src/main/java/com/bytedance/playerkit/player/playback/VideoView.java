 ...
 */
public class VideoView extends RatioFrameLayout implements Dispatcher.EventListener {
    ...
    private DisplayView.SurfaceListener mSurfaceListener;
        ...
        switch (event.code()) {
            ...
            case PlayerEvent.Action.SET_SURFACE: {
                ...
                if (player.isInPlaybackState()) {
                    mDisplayModeHelper.setVideoSize(player.getVideoWidth(), player.getVideoHeight());
                    mDisplayModeHelper.setSampleAspectRatio(player.getVideoSampleAspectRatio());
                } else {
            ...
            case PlayerEvent.Info.TRACK_INFO_READY: {
                ...
                if (first != null) {
                    ...
                    if (width > 0 && height > 0) {
                        mDisplayModeHelper.setVideoSize(width, height);
                    }
                }
                break;
            }
            case PlayerEvent.Info.VIDEO_SIZE_CHANGED: {
                InfoVideoSizeChanged videoSizeChanged = event.cast(InfoVideoSizeChanged.class);
                mDisplayModeHelper.setVideoSize(videoSizeChanged.videoWidth, videoSizeChanged.videoHeight);
                break;
            }
            case PlayerEvent.Info.VIDEO_SAR_CHANGED: {
                InfoVideoSARChanged videoSARChanged = event.cast(InfoVideoSARChanged.class);
                mDisplayModeHelper.setSampleAspectRatio(videoSARChanged.num / (float) videoSARChanged.den);
                break;
            }
        }
    }

    void setSurfaceListener(DisplayView.SurfaceListener surfaceListener) {
        if (mDisplayView != null) {
            mDisplayView.setSurfaceListener(surfaceListener);
        }
    ...
    public void selectDisplayView(@DisplayView.DisplayViewType int viewType) {
        ...
        if (mDisplayView == null) {
            ...
            mDisplayView.setSurfaceListener(mSurfaceListener);
            if (mListeners != null) {
                for (VideoViewListener listener : mListeners) {
    ...
    /**
     * Pauses playback when player in {@link Player#isInPlaybackState()}. Otherwise, stop playback
     * by calling {@link PlaybackController#stopPlayback()}
     *
     * @see PlaybackController#stopPlayback()
     * @see Player#pause()
     */
    ...
    public void bindDataSource(@NonNull MediaSource source) {
        ...
        mDisplayModeHelper.setSampleAspectRatio(0);
    }
