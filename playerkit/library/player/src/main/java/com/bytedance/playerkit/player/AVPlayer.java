...
public class AVPlayer extends ExtraObject implements Player {
    ...
    public AVPlayer(Context context,
        ...
        mDispatcher = new Dispatcher(eventLooper);
        mProgressTracker = new ProgressTracker(eventLooper, listener);
        mPlayer = playerFactory.create();
    ...
    private static class Listener implements PlayerAdapter.Listener, ProgressTracker.ProgressTaskListener {
        ...
        public void onCompletion(@NonNull PlayerAdapter mp) {
            ...
            player.mProgressTracker.stopTrackingProgress();
        ...
        public void onInfo(@NonNull PlayerAdapter mp, int what, int extra) {
            ...
            switch (what) {
                case PlayerAdapter.Info.MEDIA_INFO_VIDEO_RENDERING_START: {
                    ...
                    player.mProgressTracker.startTrackingProgress();
                    break;
                }
        ...
        public long onTrackingProgress() {
            ...
            if (duration >= 0 && currentPosition >= 0) {
                ...
                long delay = Math.min(Math.max(duration / 1000, 100), 500);
        ...
        public void onTrackInfoReady(@NonNull PlayerAdapter mp, @TrackType int trackType, @NonNull List<Track> tracks) {
            ...
            // only support video/audio track for now
            if (trackType == TRACK_TYPE_VIDEO || trackType == TRACK_TYPE_AUDIO) {
                player.selectTrack(trackType, null);
            }
        }
    ...
    public void prepare(@NonNull MediaSource source) throws IllegalStateException {
        ...
        mDispatcher.obtain(ActionPrepare.class, this).dispatch();
    ...
    public void selectTrack(@TrackType int trackType, @Nullable Track track)
        ...
        final Track target = track == null ? selectTrack(trackType) : track;
    ...
    @Nullable
    private Track selectTrack(@TrackType int trackType) {
        final List<Track> tracks = getTracks(trackType);
        if (tracks == null || tracks.isEmpty()) return null;
        return mTrackSelector.selectTrack(TrackSelector.TYPE_PLAY, trackType, tracks);
    }
    ...
    public long getCurrentPosition() {
        if (checkIsRelease("getBufferPercentage")) return 0L;
    ...
    @Override
    public String dump() {
        return "[" + L.obj2String(this) + " state:" + mapState(mState) + " " + L.obj2String(mPlayer) + "]";
    }
