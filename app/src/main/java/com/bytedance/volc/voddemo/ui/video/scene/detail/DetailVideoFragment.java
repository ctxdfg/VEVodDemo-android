...
public class DetailVideoFragment extends VideoFragment {
    ...
    protected VideoViewTransitionContract mTransitionContract;

    public void setContact(VideoViewTransitionContract transitionContract) {
        mTransitionContract = transitionContract;
    }

    public interface VideoViewTransitionContract {

        VideoView getSharedVideoView();

        void takeOverVideoView(VideoView videoView);

        Rect videoViewTransitionRect();

        void givebackVideoView(VideoView videoView);
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (mTransitionContract != null) {
            mSharedVideoView = mTransitionContract.getSharedVideoView();
    ...
    private void giveBackSharedVideoView() {
        if (mSharedVideoView.getParent() != null) {
            ViewGroup parent = (ViewGroup) mSharedVideoView.getParent();
            if (parent != null) {
                parent.removeView(mSharedVideoView);
            }
        }
        mTransitionContract.givebackVideoView(mSharedVideoView);
        mSharedVideoView = null;
        mTransitionContract = null;
    }

    private void takeOverSharedVideoView() {
        mTransitionContract.takeOverVideoView(mSharedVideoView);
        mVideoView = mSharedVideoView;
    }

    private void startEnterTransition() {
        final Rect startRect = mTransitionContract.videoViewTransitionRect();
    ...
    private void startExitTransition(Animator.AnimatorListener listener) {
        ...
        final Rect targetRect = mTransitionContract.videoViewTransitionRect();
