...
package com.bytedance.volc.voddemo.data.page;
...
public class Book<T> {
    ...
    public List<T> first(Page<T> page) {
        this.end = false;
        pages.clear();
        pages.add(page);
        return page.list;
    ...
    public boolean hasMore() {
        ...
        if (!pages.isEmpty()) {
            ...
            if (last.total == Page.TOTAL_INFINITY) {
                if (last.list == null || last.list.isEmpty() || last.list.size() < pageSize) {
                    return false;
                }
                return true;
            } else {
